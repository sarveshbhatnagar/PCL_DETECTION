...
class DontPatronizeMe:
	...
	def load_task1(self,type=''):
		...
		if type=='':
			self.train_task1_df = pd.read_csv(self.train_path+'\dontpatronizeme_pcl.csv')
		elif type=='no_punct':
			self.train_task1_df = pd.read_csv(self.train_path + '\dontpatronizeme_pcl_no_punct.csv')
		elif type=='no_redundant':
			self.train_task1_df = pd.read_csv(self.train_path + '\dontpatronizeme_pcl_no_punct_no_redundant.csv')
		else:
	...
	def load_task2(self, return_one_hot=True,type=''):
		...
		# 												])
		if type =='':
			self.train_task2_df = pd.read_csv(self.train_path+'dontpatronizeme_categories_combined_labels.csv')
		elif type =='no_punct':
			self.train_task2_df = pd.read_csv(self.train_path+'dontpatronizeme_categories_combined_labels_no_punct.csv')
		elif type =='no_redundant':
			self.train_task2_df = pd.read_csv(self.train_path+'dontpatronizeme_categories_combined_labels_no_redundant.csv')
		elif type =='seperate':
			self.train_task2_df = pd.read_csv(self.train_path+'dontpatronizeme_categories_seperate_labels.csv')
		else:
			raise ('Unsupported type parameter')
